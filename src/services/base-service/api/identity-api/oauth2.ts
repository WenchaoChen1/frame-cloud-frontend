...
export async function oauth2Token(params: APIIdentity.Oauth2TokenParamsDataType) {
  return request<APIIdentity.Oauth2TokenResultDataType>(`${process.env.FRAME_IDENTITY_SERVICE}/oauth2/token`, {
    method: 'POST',
...
export async function signOut(data: APIIdentity.signOutParams) {
  return request(`${process.env.FRAME_IDENTITY_SERVICE}/oauth2/sign-out`, {
    method: 'POST',
    data: data,
  });
}
