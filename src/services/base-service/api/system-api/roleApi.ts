...
export async function getAllTenantMenuIdByRoleId(roleId: string) {
  return request(`${process.env.SYSTEM_SERVICE}/v1/role/get-all-tenant-menu-id-by-role-id/${roleId}`, {
    method: 'GET'
  });
}
...
export async function getRoleManageDetail(id: string) {
  return request(`${process.env.SYSTEM_SERVICE}/v1/role/get-role-manage-detail/${id}`, {
    method: 'GET'
  });
}
...
export async function getRoleManageBusinessPermissionTree(tenantId: string) {
  return request(`${process.env.SYSTEM_SERVICE}/v1/role/get-role-manage-business-permission-tree/${tenantId}`, {
    method: 'GET'
  });
}

export async function getAllBusinessPermissionIdByRoleId(roleId: string) {
  return request(`${process.env.SYSTEM_SERVICE}/v1/role/get-all-business-permission-id-by-role-id/${roleId}`, {
    method: 'GET'
  });
}
